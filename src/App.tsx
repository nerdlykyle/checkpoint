...
function knownFreeGame(steamAppId?: string, title?: string) {
  return Boolean((steamAppId && KNOWN_FREE_STEAM_APP_IDS.has(steamAppId)) || (title && KNOWN_FREE_GAME_TITLES.has(title.trim().toLowerCase())))
}

function isFreeGame(game: Pick<Game, 'isFree' | 'steamAppId' | 'title'>) {
...
function getStoredRecommendationFeedback(): RecommendationFeedback {
  try {
    ...
    return Object.fromEntries(Object.entries(value).map(([steamAppId, entry]) => [steamAppId, {
      ...entry,
      excludedAt: entry.excludedAt ?? (entry.downvotes.length > 0 ? '1970-01-01T00:00:00.000Z' : undefined),
    }]))
  } catch {
    return {}
  }
...
function gameNightCalendarState(gameNight: GameNight, crew: Member[]) {
  ...
  const responses = crew.map((member) => gameNight.responses[member.id]).map((response) => response && (response.responseVersion ?? 1) === version ? response : undefined)
...
function cleanRemoteGames(games: Game[]) {
  const withoutPlaceholders = games.filter((game) => !LEGACY_PLACEHOLDER_IDS.has(game.id)).map(migrateLegacyGame)
...
function AddGameModal({ onClose, onAdd, games, defaultParentId }: { onClose: () => void; onAdd: (game: Game) => void; games: Game[]; defaultParentId?: string }) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    onAdd({
      ...
      isFree: knownFreeGame(catalogGame?.steamAppId, cleanTitle),
...
function RecommendationCard({ game, deal, onAdd, onDownvote }: { game: Recommendation; deal?: GameDeal; onAdd: (status: GameStatus) => void; onDownvote: () => void }) {
  return <article className="recommendation-card">
    ...
    <div className="recommendation-copy"><div className="recommendation-meta"><span>{game.reviewSummary}</span><strong>{game.positivePercent}%</strong><small>{game.totalReviews.toLocaleString()} reviews</small></div>
      ...
      <div className="recommendation-price">{game.isFree ? <a href={game.steamUrl} target="_blank" rel="noreferrer"><BadgeDollarSign size={15} /><span><strong>Free to play</strong> on Steam</span></a> : deal ? <a href={deal.dealUrl} target="_blank" rel="noreferrer"><BadgeDollarSign size={15} /><span><strong>${deal.price.toFixed(2)}</strong> at {deal.storeName}</span>{deal.savingsPercent >= 1 && <em>{Math.round(deal.savingsPercent)}% off</em>}</a> : <a href={game.steamUrl} target="_blank" rel="noreferrer"><Gamepad2 size={15} /> Check price on Steam</a>}<a className="recommendation-steam-link" href={game.steamUrl} target="_blank" rel="noreferrer" aria-label={`Open ${game.title} on Steam`}><ExternalLink size={15} /></a></div>
      <div className="recommendation-actions"><button type="button" onClick={() => onAdd('up-next')}><BookOpen size={14} /> Up next</button><button type="button" onClick={() => onAdd('wishlist')}><Heart size={14} /> Wishlist</button><button className="recommendation-downvote" type="button" onClick={onDownvote} aria-label={`Reject ${game.title} and remove it from recommendations`}><ThumbsDown size={15} /> <span>Remove</span></button></div>
    </div>
  </article>
}

function RecommendationsPage({ feed, loading, error, visible, feedback, deals, onAdd, onDownvote, onRestore }: { feed: RecommendationFeed | null; loading: boolean; error: string | null; visible: Recommendation[]; feedback: RecommendationFeedback; deals: Record<string, GameDeal>; onAdd: (game: Recommendation, status: GameStatus) => void; onDownvote: (game: Recommendation) => void; onRestore: (steamAppId: string) => void }) {
  ...
  const shown = (freeOnly ? visible.filter((game) => game.isFree || knownFreeGame(game.steamAppId, game.title)) : visible).slice(0, 25)
  ...
  return <div className="page recommendations-page">
    ...
    <div className="recommendation-feed-filters" role="group" aria-label="Recommendation filters"><button className={!freeOnly ? 'active' : ''} type="button" onClick={() => setFreeOnly(false)}>All picks</button><button className={freeOnly ? 'active' : ''} type="button" onClick={() => setFreeOnly(true)}><BadgeDollarSign size={14} /> Free to play</button></div>
    {showExcluded && <section className="excluded-games"><div className="section-heading"><div><span className="eyebrow">Permanent memory</span><h2>Excluded games</h2></div></div>{excluded.length ? <div className="excluded-list">{excluded.map(([steamAppId, entry]) => <div key={steamAppId}><span className="excluded-game-mark"><ThumbsDown size={15} /></span><div><strong>{entry.title}</strong><small>Rejected by {entry.downvotes.length} crew {entry.downvotes.length === 1 ? 'member' : 'members'}</small></div><button type="button" onClick={() => onRestore(steamAppId)}><RotateCcw size={14} /> Restore</button></div>)}</div> : <p className="excluded-empty">Nothing has been excluded. One thumbs-down removes a recommendation for the whole crew.</p>}</section>}
    {loading ? <div className="recommendations-loading"><RefreshCw className="spin" size={24} /><h2>Finding this week’s co-op games…</h2></div> : error ? <div className="empty-state"><CircleHelp size={28} /><h2>Recommendations need a retry</h2><p>{error}</p></div> : shown.length ? <div className="recommendation-grid">{shown.map((game) => <RecommendationCard game={game} deal={deals[game.steamAppId]} onAdd={(status) => onAdd(game, status)} onDownvote={() => onDownvote(game)} key={game.steamAppId} />)}</div> : <div className="empty-state"><Check size={28} /><h2>{freeOnly ? 'No free picks in this batch' : 'You’ve reviewed every current pick'}</h2><p>{freeOnly ? 'Switch to All picks or check again after Friday’s refresh.' : 'A new batch arrives Friday at noon Central.'}</p></div>}
  </div>
}

function ScheduleGameNightModal({ games, defaultDate, existing, onClose, onSave }: { games: Game[]; defaultDate?: string; existing?: GameNight; onClose: () => void; onSave: (night: GameNightDraft) => void }) {
  ...
  const existingStart = existing ? new Date(existing.startAt) : initialStart
  const existingEnd = existing ? new Date(existing.endAt) : initialEnd
  const [title, setTitle] = useState(existing?.title ?? 'Game Night')
  const [gameQuery, setGameQuery] = useState(() => existing?.gameTitle ?? games.find((game) => game.status === 'playing')?.title ?? '')
  const [date, setDate] = useState(defaultDate || localDateValue(existingStart))
  const [startTime, setStartTime] = useState(localTimeValue(existingStart))
  const [endTime, setEndTime] = useState(localTimeValue(existingEnd))
  ...
  function submit(event: FormEvent) {
    ...
    onSave({ title: title.trim() || 'Game Night', gameId: game?.id, gameTitle: game?.title || selectedTitle || undefined, startAt, endAt, note: note.trim() })
  }
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="modal schedule-modal" onMouseDown={(event) => event.stopPropagation()} role="dialog" aria-modal="true">
    <div className="modal-heading"><div><span className="eyebrow">{existing ? 'Change of plans' : 'Roll call'}</span><h2>{existing ? 'Edit game night' : 'Schedule a game night'}</h2></div><button className="icon-button" type="button" onClick={onClose} aria-label="Close"><X size={19} /></button></div>
    <form onSubmit={submit}>
      ...
      {existing && <p className="schedule-edit-note">Changing the game or time clears the ready check so everyone can RSVP to the updated plan. Existing personal calendar copies can be updated afterward.</p>}
      {error && <p className="calendar-form-error">{error}</p>}
      <div className="modal-actions"><button className="button button-secondary" type="button" onClick={onClose}>Cancel</button><button className="button button-primary" type="submit"><CalendarDays size={17} /> {existing ? 'Save changes' : 'Schedule'}</button></div>
    </form>
  </section></div>
}
...
function GameNightCard({ gameNight, currentUserId, crew, syncing, onEdit, onAccept, onDecline, onSyncCalendar, onCopyDiscord }: { gameNight: GameNight; currentUserId: string; crew: Member[]; syncing: boolean; onEdit: () => void; onAccept: () => void; onDecline: () => void; onSyncCalendar: () => void; onCopyDiscord: () => void }) {
  ...
  return <article className="game-night-card">
    ...
    <div className="game-night-main"><div className="game-night-heading"><div><span className="game-night-time">{start.toLocaleDateString('en-US', { weekday: 'long' })} · {start.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}–{end.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}</span><h3>{gameNight.title}</h3></div><button className="game-night-edit" type="button" onClick={onEdit}><Pencil size={13} /> Edit</button></div>{gameNight.gameTitle && <p className="game-night-game"><Gamepad2 size={13} /> {gameNight.gameTitle}</p>}{gameNight.note && <p className="game-night-note">{gameNight.note}</p>}
...
function SessionRecapModal({ session, game, onClose, onSave }: { session: GameSession; game?: Game; onClose: () => void; onSave: (progress: number, recap: string, nextObjective: string) => void }) {
  ...
  const [recap, setRecap] = useState(session.note ?? '')
...
function SessionNotesModal({ session, onClose, onSave }: { session: GameSession; onClose: () => void; onSave: (note: string) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="modal session-notes-modal" onMouseDown={(event) => event.stopPropagation()} role="dialog" aria-modal="true">
    <div className="modal-heading"><div><span className="eyebrow">Session history</span><h2>{savedNote ? 'Edit session notes' : 'Add session notes'}</h2></div><button className="icon-button" type="button" onClick={onClose} aria-label="Close"><X size={19} /></button></div>
    ...
    <label className="field field-full"><span>Session notes</span><textarea rows={8} maxLength={5000} value={note} onChange={(event) => setNote(event.target.value)} placeholder="Add clues, discoveries, decisions, or anything else worth remembering…" autoFocus /></label>
    <div className="modal-actions"><button className="button button-secondary" type="button" onClick={onClose}>Cancel</button><button className="button button-primary" type="button" disabled={nextNote === savedNote} onClick={() => onSave(nextNote)}><Check size={16} /> Save notes</button></div>
  </section></div>
}
...
function SessionHistoryEntry({ session, number, crew, initiallyOpen, onEditGame, onEditNotes }: { session: GameSession; number: number; crew: Member[]; initiallyOpen: boolean; onEditGame: () => void; onEditNotes: () => void }) {
  ...
  return <details className="session-history-entry" open={open} onToggle={(event) => setOpen(event.currentTarget.open)}>
    ...
    <div className="session-history-body"><div className="session-history-crew">{participants.map((member) => <Avatar id={member.id} small key={member.id} />)}<span>{participants.map((member) => member.name).join(', ') || 'Crew not recorded'}</span></div>
      ...
      <div className="session-history-actions"><button className="session-history-edit" type="button" onClick={onEditNotes}><NotebookPen size={13} /> {notes ? 'Edit notes' : 'Add notes'}</button><button className="session-history-edit secondary" type="button" onClick={onEditGame}><Pencil size={13} /> Change game</button></div>
    </div>
  </details>
}
...
function TonightPage({ game, event, activeSession, sessionHistory, crew, now, onBack, onStart, onPause, onResume, onFinish, onPuzzle, onCopyDiscord, onUpdateSessionNote, onEditSession, onEditSessionNotes }: { game?: Game; event?: GameNight; activeSession?: GameSession; sessionHistory: GameSession[]; crew: Member[]; now: number; onBack: () => void; onStart: () => void; onPause: () => void; onResume: () => void; onFinish: () => void; onPuzzle: () => void; onCopyDiscord: () => void; onUpdateSessionNote: (note: string) => void; onEditSession: (sessionId: string) => void; onEditSessionNotes: (sessionId: string) => void }) {
  ...
  const countdownMilliseconds = eventStart ? eventStart.getTime() - now : 0
  return <div className="page tonight-page">
    ...
    <div className="tonight-grid">
      <section className="tonight-card live-session-notes"><div className="section-heading"><div><span className="eyebrow">This session</span><h2>Live session notes</h2></div><NotebookPen size={19} /></div>
...
function App() {
  ...
  const playingSession = playing && activeSession?.gameId === playing.id ? activeSession : undefined
  const todayDate = localDateValue(new Date())
  useEffect(() => {
    const hasGameNightToday = gameNights.some((night) => localDateValue(new Date(night.startAt)) === todayDate)
  ...
  const availableRecommendations = useMemo(() => {
    ...
    return (recommendationFeed?.recommendations ?? []).filter((game) => !recommendationFeedback[game.steamAppId]?.excludedAt
      ...
      && !trackedTitles.has(game.title.trim().toLocaleLowerCase()))
  ...
  const filteredGames = useMemo(() => games.filter((game) => {
    ...
    if (smartFilter === 'free') return isFreeGame(game)
    return true
  ...
  function openStartSession(gameId?: string) { setSessionStartGameId(gameId ?? null); setShowStartSession(true) }
  function closeStartSession() { setShowStartSession(false); setSessionStartGameId(null) }
  ...
  function acceptGameNight(gameNight: GameNight) {
    ...
    setGameNights((current) => current.map((night) => night.id === gameNight.id ? { ...night, responses: { ...night.responses, [currentUser]: { ...priorResponse, status: 'accepted', respondedAt, responseVersion: night.version ?? 1, suggestedStartAt: undefined, suggestedEndAt: undefined } } } : night))
  ...
  async function syncGameNightToPersonalCalendar(gameNight: GameNight) {
    ...
    try {
      ...
      setGameNights((current) => current.map((night) => night.id === gameNight.id ? { ...night, responses: { ...night.responses, [currentUser]: { ...night.responses[currentUser], status: 'accepted', respondedAt: night.responses[currentUser]?.respondedAt || new Date().toISOString(), responseVersion: night.version ?? 1, googleEventId: result.id, calendarSyncedAt: new Date().toISOString(), calendarVersion: night.version ?? 1, suggestedStartAt: undefined, suggestedEndAt: undefined } } } : night))
  ...
  function declineGameNight(gameNightId: string, suggestedStartAt: string, suggestedEndAt: string) {
    setGameNights((current) => current.map((night) => night.id === gameNightId ? { ...night, responses: { ...night.responses, [currentUser]: { ...night.responses[currentUser], status: 'declined', respondedAt: new Date().toISOString(), responseVersion: night.version ?? 1, suggestedStartAt, suggestedEndAt } } } : night))
    setDeclineNightId(null)
    flash('Thumbs-down and new time shared with the crew')
  }
  function scheduleGameNight(draft: GameNightDraft) {
  ...
  function saveGameNight(draft: GameNightDraft) {
    if (!editingGameNight) {
      scheduleGameNight(draft)
      return
    }
    const planChanged = editingGameNight.title !== draft.title
      || editingGameNight.gameId !== draft.gameId
    ...
    setShowSchedule(false)
    setScheduleDate(undefined)
    flash(planChanged ? 'Game night updated — the crew can RSVP to the new plan' : 'Game night details updated')
  }
  ...
  function updateActiveSessionNote(note: string) {
    if (!activeSession) return
    setSessions((current) => current.map((session) => session.id === activeSession.id ? { ...session, note, updatedAt: new Date().toISOString() } : session))
  }
  ...
  function saveSessionNotes(sessionId: string, note: string) {
    const before = sessions.find((session) => session.id === sessionId)
    if (!before) { setEditingSessionNotesId(null); return }
    const previousNote = before.note?.trim() ?? ''
    if (previousNote === note) { setEditingSessionNotesId(null); return }
    const after: GameSession = { ...before, note, updatedAt: new Date().toISOString() }
    setSessions((current) => current.map((session) => session.id === sessionId ? after : session))
    const action = note ? (previousNote ? 'edited' : 'added') : 'cleared'
    recordActivity(`session-notes-${action}`, `${before.gameTitle} session notes ${action}`, [{ entity: 'session', entityId: sessionId, before, after }])
    setEditingSessionNotesId(null)
    flash(note ? 'Session notes saved for the crew' : 'Session notes cleared')
  }
  ...
  const gameNightCountdown = tonightEvent ? new Date(tonightEvent.startAt).getTime() - nowTick : 0
  ...
  return (
    ...
    <div className="app-shell">
          ...
          <section className="dashboard-grid">
            <div className="now-playing-panel"><div className="section-heading inverse"><div><span className="eyebrow">Continue playing</span><h2>Current campaign</h2></div><button className="ghost-icon" onClick={() => playing && setSelectedId(playing.id)}><MoreHorizontal size={20} /></button></div>
              {playing ? <div className="playing-content"><Cover game={playing} size="large" /><div className="playing-copy"><div className="live-pill"><span /> In progress</div><h2>{playing.title}</h2><p className="playing-meta">{playing.contentType === 'dlc' && playing.parentGameTitle ? `DLC for ${playing.parentGameTitle} · ` : ''}{[playing.genre, playing.platform, playing.year].filter(Boolean).join(' · ')}</p>
                <div className="progress-block"><div><span>Group progress</span><strong>{playing.progress}%</strong></div><div className="progress-track"><span style={{ width: `${playing.progress}%` }} /></div></div></div>
                <div className={`session-note ${expandedCampaignNoteId === playing.id ? 'is-open' : ''}`}><NotebookPen size={18} /><div><button className="session-note-toggle" type="button" onClick={() => setExpandedCampaignNoteId((current) => current === playing.id ? null : playing.id)} aria-expanded={expandedCampaignNoteId === playing.id}><span>{playingSession ? 'Live session notes' : 'Shared campaign note'}</span><ChevronDown size={14} /></button>{expandedCampaignNoteId === playing.id ? <textarea value={playingSession?.note ?? playing.note} maxLength={5000} onChange={(event) => { const note = event.target.value; if (playingSession) updateActiveSessionNote(note); else setGames((current) => current.map((game) => game.id === playing.id ? { ...game, note } : game)) }} placeholder={playingSession ? 'Clues, discoveries, and moments to remember…' : 'Where did we leave off? Add clues, plans, or the next objective…'} rows={4} autoFocus /> : <p>{playingSession ? playingSession.note || 'Add notes for this session…' : playing.note || 'Add a shared note for the next session…'}</p>}</div></div>
                <div className="playing-actions"><button className="button button-light" onClick={() => activeSession ? setView('tonight') : openStartSession(playing.id)}>{activeSession ? <Timer size={17} /> : <Play size={17} fill="currentColor" />} {activeSession ? 'Open live session' : 'Start session'}</button><button className="button campaign-puzzle-button" type="button" onClick={() => setPuzzleGameId(playing.id)}><Puzzle size={16} /> Puzzle Board</button><button className="button button-dark-ghost" onClick={() => setSelectedId(playing.id)}>View details</button></div>
      ...
      {editingSessionNotes && <SessionNotesModal session={editingSessionNotes} onClose={() => setEditingSessionNotesId(null)} onSave={(note) => saveSessionNotes(editingSessionNotes.id, note)} />}
